...
class VOC2007Soda(Base):
    ...
    CATEGORY_TO_LABEL_DICT = {
        'background': 0,
        'pepsi': 1, 'pepsi_zero': 2, 'pepsi_cherry': 3, 'mtn_dew': 4
    }
    ...
    def __init__(self, path_to_data_dir: str, mode: Base.Mode, image_min_side: float, image_max_side: float):
        ...
        path_to_voc2007_dir = os.path.join(self._path_to_data_dir, 'VOCdevkit', 'VOC2007Soda')
        ...
        for image_id in self._image_ids:
            ...
            self._image_id_to_annotation_dict[image_id] = VOC2007Soda.Annotation(
                filename=root.find('filename').text,
                objects=[VOC2007Soda.Annotation.Object(
    ...
    def __getitem__(self, index: int) -> Tuple[str, Tensor, Tensor, Tensor, Tensor]:
        ...
        labels = [VOC2007Soda.CATEGORY_TO_LABEL_DICT[obj.name] for obj in annotation.objects if not obj.difficult]
        ...
        if self._mode == VOC2007Soda.Mode.TRAIN and random.random() > 0.5:
            image = ImageOps.mirror(image)
            bboxes[:, [0, 2]] = image.width - bboxes[:, [2, 0]]  # index 0 and 2 represent `left` and `right` respectively

        image, scale = VOC2007Soda.preprocess(image, self._image_min_side, self._image_max_side)
    ...
    def evaluate(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]) -> Tuple[float, str]:
        ...
        path_to_voc2007_dir = os.path.join(self._path_to_data_dir, 'VOCdevkit', 'voc2017')
        path_to_main_dir = os.path.join(path_to_voc2007_dir, 'ImageSets', 'Main')
        path_to_annotations_dir = os.path.join(path_to_voc2007_dir, 'Annotations')

        class_to_ap_dict = {}
        for c in range(1, VOC2007Soda.num_classes()):
            category = VOC2007Soda.LABEL_TO_CATEGORY_DICT[c]
            try:
                path_to_cache_dir = os.path.join('caches', 'voc2017-soda')
        ...
        detail = ''
        for c in range(1, VOC2007Soda.num_classes()):
            detail += '{:d}: {:s} AP = {:.4f}\n'.format(c, VOC2007Soda.LABEL_TO_CATEGORY_DICT[c], class_to_ap_dict[c])
    ...
    def _write_results(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]):
        ...
        for c in range(1, VOC2007Soda.num_classes()):
            class_to_txt_files_dict[c] = open(os.path.join(path_to_results_dir, 'comp3_det_test_{:s}.txt'.format(VOC2007Soda.LABEL_TO_CATEGORY_DICT[c])), 'w')
